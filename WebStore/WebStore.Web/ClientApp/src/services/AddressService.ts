  ...
  implements IAddressService
{
  async deleteAddress(id: number): Promise<void> {
    await fetch(`${this._apiUrl}/api/AddressApi/${id}`, { method: "DELETE" });
  }

  async addOrUpdateAddress(address: IAddress): Promise<IAddress> {
    const response = await fetch(`${this._apiUrl}/address/add/api/AddressApi`, {
  ...
  async getAddress(id: number): Promise<IAddress> {
    const response = await fetch(`${this._apiUrl}/api/AddressApi/${id}`, {
      method: "GET",
    });
  ...
  async getAddresses(): Promise<IAddress[]> {
    const response = await fetch(`${this._apiUrl}/api/AddressApi`, {
      method: "GET",
    });
