import React from 'react';
import './styles/styles.scss';

import { ProductListItem } from './components/ProductListItem/ProductListItem';
import { ProductList } from './components/ProductList/ProductList';
import IProduct from './models/IProduct';
import { Zad2 } from './components/zad2/Zad2';
import { Zad3 } from './components/zad3/Zad3';
import { ILink } from './components/zad3/Zad3';
import { Zad5 } from './components/Zad5/Zad5';
import { IImage, Zad7 } from './components/Zad7/Zad7';
import { Zad8First } from './components/Zad8/Zad8First';
import { Zad10First } from './components/Zad10/Zad10First';
import { Zad11 } from './components/Zad11/Zad11';

function App() {
  let products: Array<IProduct> = [
    {
      id: 1,
      name: "Dell",
      type: "Computers",
      price: 4000
    },
    {
      id: 2,
      name: "MacBook Pro 13 inch",
      type: "Computers",
      price: 7000
    },
  ]

  let links: Array<ILink> = [
    {
      id: 1,
      name: "Link 1",
      url: "https://google.com"
    },
    {
      id: 2,
      name: "Link 2",
      url: "https://google.com"
    },
    {
      id: 3,
      name: "Link 3",
      url: "https://google.com"
    },
    {
      id: 4,
      name: "Link 4",
      url: "https://google.com"
    },
  ]

  let images: Array<IImage> = [
    {
      src: 'https://cdn.pixabay.com/photo/2015/04/23/22/00/tree-736885__480.jpg'
    },
    {
      src: 'data:image/jpeg;base64,'
    },
    {
      src: 'https://cdn.pixabay.com/photo/2018/01/12/10/19/fantasy-3077928__480.jpg'
    },
    {
      src: 'https://images.ctfassets.net/hrltx12pl8hq/3KWcqxPlvmgkpMS5VdjLAk/3cbdd812faf4b8c343b259656d31a0ee/rendered_15.jpg?fit=fill&w=480&h=270'
    },
    {
      src: 'https://thumbs.dreamstime.com/b/environment-earth-day-hands-trees-growing-seedlings-bokeh-green-background-female-hand-holding-tree-nature-field-gra-130247647.jpg'
    },
  ]

  return (
    <div className="App">
      <div className="container">
        <ProductList products={products} name="Lista"/>
        <p className='section-heading spacing'>Zadanie 2</p>
        <Zad2/>
        <p className='section-heading spacing'>Zadanie 3 i 4</p>
        <Zad3 links={links}/>
        <p className='section-heading spacing'>Zadanie 5 i 6</p>
        <Zad5/>
        <p className='section-heading spacing'>Zadanie 7</p>
        <Zad7 images={images}/>
        <p className='section-heading spacing'>Zadanie 8 i 9</p>
        <Zad8First/>
        <p className='section-heading spacing'>Zadanie 10</p>
        <Zad10First/>
        <p className='section-heading spacing'>Zadanie 11</p>
        <Zad11/>
      </div>
    </div>
  );
}

export default App;
