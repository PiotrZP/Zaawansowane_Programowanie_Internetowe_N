...
import ILink from "../../model/ILinkProps";
import "../../styles/ComponentTask.css";
      ...
      <div>
        {links.map((link) => (
          <div
            ...
            className={areLinksPressed(link) ? "pressed" : "normal"}
          >
      ...
      <div>
        {links.map((link) => (
          <div
            key={link.name}
            onClick={() => addPressedLink(link)}
            className={isLinkPressed(link) ? "pressed" : "normal"}
          >
            <a rel="noopener noreferrer" href={link.url} target="_blank">
              {link.name}
            </a>
...
export default ComponentTask3and4;
